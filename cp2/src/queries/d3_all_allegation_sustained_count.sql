copy (
    SELECT ac.category, extract('year' from da.incident_date) as year, COUNT(d.allegation_id) as count
    ...
    AND d.final_finding = 'SU'
    GROUP BY ac.category, year
    ORDER BY category, year, count DESC
) TO '/Users/blaine/Desktop/d3_scatter/data/all_sustained_by_cat.csv' DELIMITER ',' CSV HEADER;
